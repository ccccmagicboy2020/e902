...
CMap <HWND,HWND,CCustomBitmapButton*,CCustomBitmapButton*> CCustomBitmapButton::m_mxbr820_reg_verify_helperaptionFrames;
...
int CCustomBitmapButton::CreateCaptionFrame(CWnd * pCaptionWnd, int nIDIcon)
{
	...
	m_cs.Lock();
	if(m_mxbr820_reg_verify_helperaptionFrames.Lookup(pCaptionWnd->m_hWnd,p))
	{
		m_cs.Unlock();
		return CBNERR_INCORRECTPARAMETER;
	}
	...
	m_mxbr820_reg_verify_helperaptionFrames.SetAt(pCaptionWnd->m_hWnd,this);
...
int CCustomBitmapButton::DestroyCaptionFrame()
{
	...
	m_mxbr820_reg_verify_helperaptionFrames.RemoveKey(m_pCaptionWnd->m_hWnd);
	m_pOldWndProc = NULL;
	m_pCaptionWnd = NULL;
	m_cs.Unlock();
...
int CCustomBitmapButton::SetTooltipText(CString sText)
{
	...
	if(GetStyle()&CBNSTYLE_CAPTION)
	{
		...
		m_cs.Lock();
		if(!m_mxbr820_reg_verify_helperaptionFrames.Lookup(m_pCaptionWnd->m_hWnd,pTooltipBn))
		{
			m_cs.Unlock();
			return CBNERR_INCORRECTBUTTONSTYLE;
		}
...
LRESULT CALLBACK CCustomBitmapButton::CaptionFrameWindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	CCustomBitmapButton* p;
	m_cs.Lock();
	if(!m_mxbr820_reg_verify_helperaptionFrames.Lookup(hWnd,p))
	{
		m_cs.Unlock();
		return 0;
	}
