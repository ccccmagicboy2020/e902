...
#define XBR820_PMU_BOOT_ADC	(0x1f010000UL + 0x58)
...
static void delay(void) {
	for (int i = 0; i < (SYSTEM_CLOCK/300); i++)
		nop();
}
#endif
...
int  flashInit(){
  ...
  sf_size = 0;
#ifdef CONFIG_LOG_NUMS
	log_data = (uint32_t *)(XBR820_SRAM_BASE + XBR820_SRAM_SIZE - sizeof(uint32_t)*CONFIG_LOG_NUMS);
	for (log_i = 0; log_i < CONFIG_LOG_NUMS; log_i++)
		log_data[log_i] = 0;
	log_i = 0;
#endif
  //reg32_write(XBR820_PMU_BOOT_ADC, 0x13); // Not boot from SPINorFlash
  write_protect(1);
  sf_id = get_id(1);
  if (!sf_id)
	return ERROR_INIT;
